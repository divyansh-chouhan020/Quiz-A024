...
function checkImageExists(url) {
    ...
    return new Promise((resolve)=>{
        img.onload = function() {
            // Image loaded successfully, resource is available
            resolve(true);
        };
        
        img.onerror = function() {
            // Image failed to load, resource is not available
            resolve(false);
        };
        img.src = url;
    })
...
function extractGitHubUsername(githubUrl) {
    ...
    if (match && match[1]) {
        // Return the username converted to lowercase
...
async function fetchContributorData() {
        try {
            // Assuming the README.md file is hosted on GitHub or any public URL
            const response = await fetch('../contribution/content.md');
            const markdown = await response.text();
            console.log(markdown);

            // Extract contributors using regex (depends on the markdown structure)
            ...
            contributors.forEach(async contributor => {
                const name = contributor[1];
                    ...
                    // const avatarUrl = `${githubUrl}.png?size=100`;
                    ...
                    const exists= await checkImageExists(avatarUrl)
                        if (!exists) {
                            ...
                            // Handle the logic when the image is not available, like using a default image
                ...
                card.innerHTML = `
                    ...
                    <h3>${name}</h3>
                    <a href="${githubUrl}" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 48 48">
